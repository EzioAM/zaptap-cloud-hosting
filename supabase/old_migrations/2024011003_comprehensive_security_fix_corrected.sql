...
-- This migration addresses all security issues with correct column names
...
-- 1. FIX: Create change_history table if it doesn't exist
...
-- 2. FIX: Recreate views without SECURITY DEFINER and without exposing auth.users

-- Create change_history_view using public.users
...
WHERE ch.changed_by = auth.uid() OR EXISTS (
...
-- Create user_roles_summary view
...
-- 3. FIX: Enable RLS on all tables that are missing it
...
-- Enable RLS on automation_executions (already exists from migration)
...
-- Enable RLS on step_executions (already exists from migration)
...
-- Create shares table if it doesn't exist (different from public_shares)
...
-- Create reviews table if it doesn't exist (generic reviews table)
...
-- Create comments table if it doesn't exist (different from automation_comments)
...
-- 4. Enable RLS on change_history table
ALTER TABLE public.change_history ENABLE ROW LEVEL SECURITY;

-- Drop existing policies if they exist
DROP POLICY IF EXISTS "Admins can view change history" ON public.change_history;
DROP POLICY IF EXISTS "System can insert change history" ON public.change_history;

-- Create policy for change_history
...
-- 5. Add indexes for better performance on RLS queries
...
CREATE INDEX IF NOT EXISTS idx_change_history_changed_by ON public.change_history(changed_by);
