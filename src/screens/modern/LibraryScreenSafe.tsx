...
interface SavedAutomation {
  ...
  icon: string;
  color: string;
  steps: any[]; // Changed from number to array
  lastRun?: string;
  totalRuns: number;
  isActive: boolean;
  tags: string[];
...
const LibraryScreenSafe = () => {
  ...
  const filteredAutomations = automations?.map(automation => ({
    ...automation,
    icon: automation.icon || 'flash',
    color: automation.color || '#2196F3',
    tags: automation.tags || [],
    totalRuns: automation.totalRuns || 0,
    isActive: automation.is_active ?? automation.isActive ?? true,
    createdAt: automation.created_at || automation.createdAt
  })).filter(automation => {
    const matchesSearch = (automation.title || '').toLowerCase().includes(searchQuery.toLowerCase()) ||
                         (automation.description || '').toLowerCase().includes(searchQuery.toLowerCase());
    const isActive = automation.is_active ?? automation.isActive ?? true;
  ...
  const renderAutomationItem = ({ item }: { item: SavedAutomation }) => (
    ...
    >
      <View style={styles.cardContent}>
        ...
        <View style={styles.cardInfo}>
          <View style={styles.titleRow}>
            ...
            <TouchableOpacity
              onPress={() => handleToggleActive(item as any)}
              style={styles.toggleButton}
            >
              <MaterialCommunityIcons 
                name={(item.is_active ?? item.isActive) ? "toggle-switch" : "toggle-switch-off"} 
                size={28} 
                color={(item.is_active ?? item.isActive) ? (theme.colors?.primary || '#2196F3') : '#ccc'} 
              />
            </TouchableOpacity>
          </View>
      ...
      <View style={styles.cardActions}>
        ...
        <TouchableOpacity
          onPress={() => navigation.navigate('AutomationBuilder' as never, { automationId: item.id } as never)}
          style={styles.actionButton}
        >
