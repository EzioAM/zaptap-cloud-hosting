-- ============================================
-- ZAPTAP SHARING SYSTEM MIGRATION SCRIPT (FIXED)
-- ============================================
-- This script migrates existing databases to support the full sharing system
-- Fixed version that ensures proper column creation before indexing
...
-- Create executions table with all columns
...
-- Add missing columns to executions if table already existed
ALTER TABLE public.executions 
ADD COLUMN IF NOT EXISTS deployment_id UUID REFERENCES deployments(id);
...
-- Add missing columns to public_shares if table already existed
...
-- ============================================
-- 5. ADD CHECK CONSTRAINTS (SAFE)
-- ============================================
DO $$
BEGIN
    RAISE NOTICE '✓ Adding check constraints...';
    
    -- Add access_type constraint
    IF NOT EXISTS (
    ...
    -- Add deployment_type constraint
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint 
        WHERE conname = 'deployments_deployment_type_check'
    ) THEN
        ALTER TABLE public.deployments 
        ADD CONSTRAINT deployments_deployment_type_check 
        CHECK (deployment_type IN ('nfc', 'qr', 'link', 'widget'));
    END IF;
    
    -- Add share_type constraint
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint 
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint 
        WHERE conname = 'executions_status_check'
    ) THEN
        ALTER TABLE public.executions 
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint 
        WHERE conname = 'executions_source_check'
    ) THEN
        ALTER TABLE public.executions 
        ADD CONSTRAINT executions_source_check 
        CHECK (source IN ('app', 'nfc', 'qr', 'web', 'api', 'schedule'));
    END IF;
    
    -- Add method constraint
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint 
        WHERE conname = 'sharing_logs_method_check'
    ) THEN
        ALTER TABLE public.sharing_logs 
        ADD CONSTRAINT sharing_logs_method_check 
        CHECK (method IN ('link', 'qr', 'nfc', 'email', 'sms', 'social'));
    END IF;
END $$;

-- ============================================
-- 6. CREATE INDEXES (NOW SAFE)
-- ============================================
DO $$
BEGIN
...
-- Executions indexes (now safe after ensuring columns exist)
...
-- ============================================
-- 8. CREATE RLS POLICIES (SAFE VERSION)
-- ============================================
DO $$
BEGIN
...
CREATE POLICY "Users can view public automations" ON automations
    FOR SELECT USING (
        is_public = true 
        OR access_type IN ('public', 'link_only')
    );
...
-- ============================================
-- 11. ENABLE REALTIME (SAFE VERSION)
-- ============================================
DO $$
BEGIN
...
BEGIN
    ...
    AND table_name IN ('automations', 'deployments', 'public_shares', 'executions', 'sharing_logs');
    ...
    -- Check for deployment_id in executions
    SELECT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'executions' AND column_name = 'deployment_id'
    ) INTO col_exists;
    
    IF NOT col_exists THEN
        missing_cols := missing_cols || 'executions.deployment_id, ';
    END IF;
